...
void copyPartOfString(char *target, char *source, int length);
unsigned int getCountOfSpaces(const char *string);
// server's root
char* root = NULL;
...
char* indexes(const char* path) {
    ...
    char *fullPath = malloc(sizeof(char) * (pathLength + 9 + 1));
    if (fullPath == NULL) {
        free(fullPath);
        return NULL;
    }
    ...
    fullPath = malloc(sizeof(char) * (pathLength + 10 + 1));
    if (fullPath == NULL) {
        free(fullPath);
        return NULL;        
    }
...
bool load(FILE* file, BYTE** content, size_t* length) {
    ...
    *length = 0;
    ...
    while((bytes = fread(buffer, sizeof(BYTE), BYTES, file)) != EOF) {
        if (bytes == 0)
            break;
...
const char* lookup(const char* path) {
    ...
    if ((dotLink = strrchr(path, '.')) != NULL) {
        ...
        copyPartOfString(extension, dotLink, pathLength);
        //printf("%s\n", extension);
...
bool parse(const char* line, char* abs_path, char* query) {
    // TODO
    char *firstSpaceLink = strchr(line, ' ');
    char *lastSpaceLink = strrchr(line, ' ');
    int lineLength = strlen(line);
    char *httpVersionLink = strstr(line, "HTTP/");
    if (lineLength < 4 || firstSpaceLink == NULL || getCountOfSpaces(line) != 2 || httpVersionLink == NULL
        || line[lineLength - 2] != '\r' || line[lineLength - 1] != '\n' || strchr(line, '"') != NULL || httpVersionLink < lastSpaceLink) {
        error(400);
        return false;
    }
    ...
    unsigned long firstSpaceIndex = (unsigned long) firstSpaceLink;
    unsigned long lastSpaceIndex = (unsigned long) lastSpaceLink;
    char *requestTarget = malloc(sizeof(char) * lastSpaceIndex - firstSpaceIndex);
    ...
    copyPartOfString(requestTarget, firstSpaceLink, lastSpaceIndex - firstSpaceIndex);
    if (requestTarget[0] != '/') {
        error(501);
        return false;  
    }
    ...
    char *enterLink = strrchr(line, '\r');
    unsigned long enterIndex = (unsigned long) enterLink;
    char *httpVersion = malloc(sizeof(char) * (enterIndex - lastSpaceIndex));
    if (httpVersion == NULL) {
        free(httpVersion);
        error(400);
        return false;
    ...
    copyPartOfString(httpVersion, lastSpaceLink, enterIndex - lastSpaceIndex);
    ...
    char *queryLink = strchr(requestTarget, '?');
    if (queryLink == NULL) {
        strcpy(abs_path, requestTarget);
        query[0] = '\0';
    }
    else {
        queryLink = strchr(line, '?');
        unsigned long queryIndex = (unsigned long) queryLink;
        char *queryPart = malloc(sizeof(char) * (lastSpaceIndex - queryIndex));
        copyPartOfString(queryPart, queryLink, lastSpaceIndex - queryIndex);
        strcpy(query, queryPart);
        free(queryPart);
        
        char* absolutePath = malloc(sizeof(char) * (queryIndex - firstSpaceIndex));
        copyPartOfString(absolutePath, firstSpaceLink, queryIndex - firstSpaceIndex);
